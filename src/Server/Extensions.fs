...
let rec findParent (directory: string) (fileToFind: string) =
    ...
    if files.Any(fun file -> Path.GetFileName(file).ToLower() = fileToFind.ToLower()) then
        path
    else
        findParent (DirectoryInfo(path).Parent.FullName) fileToFind

let solutionRoot () =
    findParent __SOURCE_DIRECTORY__ "App.sln"
