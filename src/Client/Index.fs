...
let update msg model =
    ...
    | GetReport ->
        ...
        | None -> { model with ServerState = Loading }, Cmd.OfAsync.either getResponse model.Postcode GotReport ErrorMsg
        | _ -> model, Cmd.none
                ...
                Bulma.image [
                    prop.children [
                        Html.img [
                            ...
                            prop.src (
                                sprintf
                                    "https://raw.githubusercontent.com/erikflowers/weather-icons/master/svg/wi-%s.svg"
                                    weatherReport.WeatherType.IconName
                            )
                        ]
                    ]
                ]
                    ...
                    prop.children [
                        Html.tbody [
                            Html.tr [
                                ...
                                Html.td (sprintf "%.2f degrees" weatherReport.Temperature)
                            ]
                        ]
                    ]
                ]
    ...
    let locationWidget model =
        widget "Location" [
            Bulma.table [
                ...
                prop.children [
                    Html.tbody [
                        Html.tr [ Html.th "Region"; Html.td model.Location.Location.Region ]
                    ...
                    Bulma.field.div [
                        ...
                        prop.children [
                            Bulma.control.div [
                                ...
                                prop.children [
                                    Bulma.input.text [
                                        if model.ValidationError.IsSome then
                                            color.isDanger
                                        else
                                            color.isInfo
                                    ...
                                    Bulma.help [
                                        if model.ValidationError.IsNone then
                                            color.isPrimary
                                        else
                                            color.isDanger
